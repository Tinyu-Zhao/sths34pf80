/* Includes ------------------------------------------------------------------*/
#include "./include/lpsths34pf80.h"
...
esp_err_t sths34pf80_init(lpsths34pf80_t *const me, i2c_port_t i2c_num, uint8_t dev_addr)
{
    ...
    me->stmdev_ctx.write_reg = (stmdev_write_ptr)i2c_write;
    me->stmdev_ctx.read_reg  = (stmdev_read_ptr)i2c_read;
    ...
    sths34pf80_avg_tobject_num_get(&me->stmdev_ctx, &avg_tmos);
    lp_core_printf("read avg_tmos: %d\r\n", avg_tmos);
    int32_t avgErr = sths34pf80_avg_tobject_num_set(&me->stmdev_ctx, STHS34PF80_AVG_TMOS_32);
    sths34pf80_avg_tobject_num_get(&me->stmdev_ctx, &avg_tmos);
    lp_core_printf("read 2 avg_tmos: %d\r\n", avg_tmos);
